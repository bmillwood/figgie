...
let create ~(config : Config.t) ~room =
  let hands, gold =
    let open Card in
    ...
    let hands =
      Array.init Params.num_players ~f:(fun _i ->
    ...
    for ix = 0 to Market.Size.to_int Params.num_cards_in_deck - 1 do
      ...
      let hand_suit = Hand.get hands.(ix mod Params.num_players) ~suit in
  ...
  let players =
    List.map2_exn
      (Map.data (Lobby.Room.players room))
      (Array.to_list hands)
      ~f:(fun p hand ->
          let player = Player.create ~username:p.username in
          player.hand <- hand;
          p.username, player
        )
